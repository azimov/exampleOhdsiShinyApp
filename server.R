...
server <- shiny::shinyServer(function(input, output, session) { 
  ...
  shiny::observeEvent(input$menu,{ 
    ...
    if(input$menu == 'CohortDiagnostics' & runServer[['CohortDiagnostics']]==1){
      diagnosticsExplorer$diagnosticsExplorerModule(
        id = 'CohortDiagnostics',
        dataSource = diagnosticsExplorer$dataSource,
        databaseTable = diagnosticsExplorer$database,
        cohortTable = diagnosticsExplorer$cohort,
        enableAnnotation = F,
        enableAuthorization = F,
        enabledTabs = diagnosticsExplorer$enabledTabs,
        conceptSets = diagnosticsExplorer$conceptSets,
